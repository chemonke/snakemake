...
rule create_database_schema:
    ...
    shell:
        ...
        docker run --rm --user $(id -u):$(id -g) --network=my_network \
            ...
            /opt/conda/envs/pydev/bin/python {scripts}/sql_create_schema.py \
            --user {params.user} \
...
rule insert_data_to_db:
    input:
        ...
        sql_sentinel=config["files"]["sql_running_sentinel"],
        schema_sentinel=config["files"]["schema_created_sentinel"],
        pydev_sentinel=config["files"]["pydev_sentinel"]
    output:
    ...
    shell:
        ...
        docker run --rm --user $(id -u):$(id -g) --network=my_network \
            -v $(pwd):/workspace -w /workspace pydev \
            /opt/conda/envs/pydev/bin/python {scripts}/sql_insert_data.py \
            --input {input.results} \
